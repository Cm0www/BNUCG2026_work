...
6. 解压下载的结果压缩包，将其中生成的文件复制到本项目的 `results/` 目录中，再提交到 GitHub。
...
运行完成后，将 `work06_results.zip` 中的文件复制到 `results/` 目录。GitHub 会自动显示下列结果图。
...
本实验优化的是源球体的顶点位置，而不是直接复制目标网格。因此，最终模型主要在多视角轮廓上逼近奶牛形状；局部细节和不可见面不一定与目标模型完全一致。

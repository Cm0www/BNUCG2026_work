...
@ti.func
def shadow_check(p, light_pos):
    """
    检测交点 p 是否处于阴影中
    p: 物体表面的交点
    light_pos: 光源位置
    返回: True 表示在阴影中，False 表示不在阴影中
    """
    in_shadow = False
    # 从交点向光源发射射线 (偏移一点避免自相交)
    shadow_ro = p + 1e-4 * normalize(light_pos - p)
    shadow_rd = normalize(light_pos - p)
    light_dist = (light_pos - p).norm()  # 交点到光源的距离
...
def render():
    for i, j in pixels:
        ...
        if min_t < 1e9:
            ...
            # 检测是否在阴影中
            in_shadow = shadow_check(p, light_pos)
...
def main():
    window = ti.ui.Window("Phong Shading with Shadows", (res_x, res_y))
